...
/// The colour of the upper estimate — the breaker-spec bound. Warmer than [`accent`] because it is
/// the ceiling of the bracket, not because anything is wrong.
pub fn ceiling(ui: &Ui) -> Color32 {
    if ui.visuals().dark_mode {
        Color32::from_rgb(255, 190, 85)
    } else {
        // Darkened with [`accent`] and for the same reason: the two figures sit side by side, and
        // one of them fading into the panel would read as the weaker estimate rather than as the
        // upper one.
        Color32::from_rgb(130, 74, 0)
    }
}
