...
- One other anomaly excludes a session: `FellInDstGap`, where a reported start or end is a wall time that never occurred — see `docs/time/README.md`, "Time zone". The resolved instant is a guess rather than a reading, which is the same objection as an inconsistent duration. Nothing else removes a session.
