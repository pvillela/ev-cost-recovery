...
Three of the nine exclude a session from every estimate — `InconsistentDuration`, `FellInDstGap` and `DstUnresolvable`. Nothing else removes a session. The latter two also leave the record with no instant to stand on: it carries the sentinels `UNPLACEABLE_START` and `UNPLACEABLE_END` in place of a start and an end — inverted on purpose, so that anything trying to place it on a timeline panics rather than answering — and its time columns are dashed in the report and left empty in a generated workbook.
...
- **`FellInDstGap`** — a reported wall time that never occurred, in the hour the clocks jump over when DST begins. It names no instant at all, so none can be assigned and the session is excluded. Shifting it to either side of the gap would be a guess presented as a reading. See `docs/time/README.md`, "Time zone".
- **`DstUnresolvable`** — a reported wall time in the hour the clocks repeat when DST ends. Such a time can be read under either offset, so the record yields more than one candidate pairing of start and end. `Conn_Duration` is ordinarily what settles it: the true pairing is the one whose elapsed time agrees with the reported duration. This kind says no candidate pairing satisfies `duration_is_consistent`. That evidence has failed — whatever `Conn_Duration` measures on this row, it is not the elapsed time the inference assumes — so nothing is left to choose with. The software declines to pick rather than assigning a reading arbitrarily, and the session is excluded. See `docs/time/README.md`, "Time zone".

  - `InconsistentDuration` is always raised alongside it, since a record whose fields agree under no reading is inconsistent however it is read.
  - Outside the fold there are no two readings to choose between, so the same disagreement raises `InconsistentDuration` alone: the reported times stand, and the session is excluded on consistency.
- **`DstAmbiguousDuplicated`** — the reported start fell in the repeated hour when the clocks go back, and *both* readings of it reproduce the reported end. Neither can be ruled out, so the record is duplicated, once under each offset, and both copies take part in every estimate. The two share a CSV row and are told apart by an `-EDT`/`-EST` suffix on the session id, which is why a generated workbook can show two rows for one row of the CSV. Where the duration does settle which reading was meant, there is no anomaly; where nothing settles it, the anomaly is `DstUnresolvable` above.
