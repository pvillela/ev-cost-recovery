...
`docs/session/site-load-report-marcus.txt` is the same table as `site_load.report.txt`, kept under
`docs/` because end users read documentation rather than fixtures. No test writes into `docs/` — a
document is something a person publishes, and a test run that rewrites one leaves a change nobody
chose in a file a reader is trusted to have read. So after regenerating the golden, copy it across
by hand:
...
— it compares the two files, while the test beside it holds the golden to the renderer, so one hop
each. It is `#[ignore]`d because it opens `docs/`, which a release build must not, so a plain
`cargo test` stays green while the copy is stale. `ci.yml` asks for it by name on every push, and
that is the only place it runs.
...
**Free constants** — declared outright, and the ones to change to describe a different
installation. [`site-specific-constants.md`](site-specific-constants.md) lists them with their
values and what each means; the values live there rather than here, so that a site being set up has
one document to work through and this one does not go stale beside it.
...
`AnomalyKind::as_str` and `Anomaly::as_str` has an entry. Both its tests open `docs/`, which a
release build must not, so both are `#[ignore]`d and a plain `cargo test` skips them. Run them
with:
...
`ci.yml` does the same on every push. The error variants are not covered at all, because their
...
`tests/docs_errors.rs` fails until it is there — under `cargo test --test docs_errors -- --ignored`,
since its tests are `#[ignore]`d and a plain run skips them.
