...
`docs/` because end users read documentation rather than fixtures. It is regenerated by the same
command and pinned by the same test file, so the two cannot drift.
