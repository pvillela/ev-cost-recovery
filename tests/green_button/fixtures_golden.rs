...
//! can go wrong unseen -- the billing-period column centred where the reference workbook
//! left-aligns it, or every row 15pt where the reference is 13.8 -- and neither shows up in a dump
//! that records only values and number formats.
...
/// one. The Python program's workbook under `data/reference/green_button/` is the opposite case —
/// never regenerated, never read by a test, kept only as provenance.
...
/// Default row height, and any row that overrides it. Either can be wrong and invisible to a dump
/// that records only cell contents.
